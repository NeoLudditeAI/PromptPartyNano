 ...
 */
export const notifyPlayerTurn = functions.database.onValueUpdated(
  '/games/{gameId}/turns',
  async (event) => {
    try {
 ...
 */
export const notifyReaction = functions.database.onValueCreated(
  '/games/{gameId}/reactions/{reactionId}',
  async (event) => {
    try {
      const gameId = event.params.gameId
 ...
 */
export const storeFCMToken = functions.https.onCall({
  cors: true,
  region: 'us-central1'
}, async (request) => {
  try {
